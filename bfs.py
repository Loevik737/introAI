...
def main():
    ...
    if solution:
        ...
        for node in OPEN.list + CLOSED:
            if node.status == "Open":
                board.nodes[node.row][node.col] = '*'
            if board.nodes[node.row][node.col] != 'A' and board.nodes[node.row][node.col] != 'B' and board.nodes[node.row][node.col] !='o' and node.status =="Closed":
                board.nodes[node.row][node.col] = 'X'
